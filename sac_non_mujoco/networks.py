...
class ActorNetwork(nn.Module):
    ...
    def forward(self, state):
        ...
        mu = self.mu(prob)
        # o com sigma = T.sigmoid(self.sigma(prob))*0.4
        # sigma = T.tanh(self.sigma(prob))*0.4

        sigma = self.sigma(prob)
        sigma = T.clamp(sigma,  min=self.reparam_noise, max=0.4)
